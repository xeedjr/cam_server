...
class JPEGReader {
    ...
    void header_buffer(std::vector<uint8_t>& jpeg);
    ...
    /// The current number of components of the image.  Valid after calling \c header().
    /// \note It is not the case that components() == colorComponents() in all cases.  
    /// When quantizing
    /// to generate an indexed image, components() == 1, but colorComponents() will
    ...
    /// The current number of color components of the image.  Valid after calling \c header().
    /// \sa components()
    ...
    /// laid out like so, if we assume for the example that colorComponents() == 3.
    /// \code
    ...
    /// Choose a good extraction scale if you ultimately want at most targetWidth x targetHeight
    ...
    /// Get the recommended number of rows to provide.  Valid after calling \c header().
    ...
    /// only that many values returned from your RowIter::operator*().  Essentially, 
    ...
    /// row.  If you do this, obviously you need to do something with the row before
    ...
    std::vector<unsigned char> cmap;            /// The color map if quantization is requested
    std::string warningMsg;                     /// All the warnings.
};

inline unsigned JPEGReader::width() const {
    return cinfo.output_width;
}

inline unsigned JPEGReader::height() const {
...
inline unsigned JPEGReader::maxRowPtrs() const {
    return max_row_ptrs;
}
...
inline const std::string& JPEGReader::warnings() const {
    return warningMsg;
}
...
void JPEGReader::load(RowPtrIter rows) {
    ...
    while (cinfo.output_scanline < cinfo.output_height) {
        assert(cinfo.output_scanline >= base && cinfo.output_scanline <= base + num_row_ptrs);
